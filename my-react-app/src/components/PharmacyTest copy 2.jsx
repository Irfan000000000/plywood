...
//           .pos-header {
//             background: linear-gradient(135deg, rgb(0, 123, 255) 0%, #059669 100%);
//             color: white;
//             padding: 1rem 1.5rem;
...
//                       <i className="fas fa-plus-circle" style={{ color: 'rgb(0, 123, 255)', fontSize: '20px' }}></i>
//                     </div>
//                   </div>
//                 );
//               })}
...
const PharmacyTest = ({
  ...
  const invoiceFormRefs = {
    ...
    full_name: fullNameRef,
    book_id: bookIdRef,
    invoice_status: invoiceStatusRef,
    advance: advanceRef,
  };
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      // Prevent browser shortcuts - SOFTWARE SHORTCUTS FIRST PRIORITY
      ...
      // Ctrl + S - Save Invoice
      if (ctrl && key === "s" && checkComponent === "pharmacy") {
        event.preventDefault();
      ...
      // Ctrl + I - Open Invoices List
      if (ctrl && key === "i" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setShow("invoice");
        resetFormData();
      ...
      if (ctrl && key === "g" && checkComponent === "pharmacy") {
      event.preventDefault();
      event.stopPropagation();
      setTimeout(() => cartSearchRef.current?.focus(), 100);
      return false;
    }

      // Ctrl + N - New Invoice (Reset)
      if (ctrl && key === "n" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setTableData([]);
        resetFormData();
        setFormDisabled(false);
        setShowCheckout(false);
        setInvoiceUpdate({ invoice_no: "" });
        setActiveBlock('invoiceForm');
        setActiveFieldIndex(0);
      ...
      // Ctrl + / - Focus Search
      if (ctrl && key === "/" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock('itemsPOS');
        setActiveFieldIndex(0);
      ...
      if (ctrl && key === "b" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock('itemsPOS');
      ...
      // Ctrl + U - Focus Phone
      if (ctrl && key === "u" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock('invoiceForm');
        setActiveFieldIndex(1);
      ...
      // Ctrl + F - Focus Full Name
      if (ctrl && key === "f" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock('invoiceForm');
      ...
      // Ctrl + X - Delete Selected Cart Item
      // if (ctrl && key === "x" && activeBlock === 'invoiceItems' && checkComponent === "pharmacy") {
      //   event.preventDefault();
      //   event.stopPropagation();
      //   const rowIndex = Math.floor(activeFieldIndex / 3);
      //   if (rowIndex >= 0 && rowIndex < tableData.length) {
      ...
      //   return false;
      // }
// Ctrl + X - Delete Selected Cart Item (Update this section)
if (ctrl && key === "x" && activeBlock === 'invoiceItems' && checkComponent === "pharmacy") {
  event.preventDefault();
  event.stopPropagation();
  const rowIndex = Math.floor(activeFieldIndex / 2); // Changed from 3 to 2
  if (rowIndex >= 0 && rowIndex < tableData.length) {
    handleDelete(rowIndex);
    toast.info("Item Deleted");
    if (activeFieldIndex >= (tableData.length - 1) * 2) { // Changed from 3 to 2
      setActiveFieldIndex(Math.max(0, activeFieldIndex - 2)); // Changed from 3 to 2
    }
  }
  return false;
}
      // Ctrl + Arrow Left - Navigate Sections Left
      if (ctrl && event.key === 'ArrowLeft' && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock(prev => {
          if (prev === 'invoiceItems') return 'itemsPOS';
      ...
      // Ctrl + Arrow Right - Navigate Sections Right
      if (ctrl && event.key === 'ArrowRight' && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock(prev => {
          if (prev === 'invoiceForm') return 'itemsPOS';
      ...
      // Ctrl + P - Print/Preview Invoice
      if (ctrl && key === "p" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
      ...
      // Ctrl + D - Delete All Cart Items (Clear Cart)
      if (ctrl && key === "d" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        if (tableData.length > 0) {
      ...
      if (ctrl && key === "h" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
      ...
      // F9 - Toggle Checkout View
      if (event.key === "F9" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        if (tableData.length > 0) {
          setShowCheckout(!showCheckout);
          // toast.info(showCheckout ? "Back to Cart" : "Checkout View");
        }
        return false;
      }

      // F2 - Toggle Payment Status (Paid/Unpaid)
      if (event.key === "F2" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        const newStatus = formData.invoice_status === "paid" ? "unpaid" : "paid";
        setFormData({
          ...formData,
          invoice_status: newStatus,
          advance: newStatus === "unpaid" ? 0 : formData.advance
        });
        toast.info(`Status: ${newStatus.toUpperCase()}`);
        return false;
      }

      // F3 - Focus Advance Payment Field
      if (event.key === "F3" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        if (showCheckout) {
          setTimeout(() => advanceRef.current?.focus(), 100);
        }
        return false;
      }

      // F5 - Refresh/Reload
      if (event.key === "F5" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        if (window.confirm("Refresh page? Unsaved data will be lost.")) {
          refreshData();
        }
        return false;
      }

      // F8 - Toggle Contact Method (Phone/WhatsApp)
      if (event.key === "F8" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        toggleContactMethod();
        return false;
      ...
      // if (!ctrl && !shift && !alt && checkComponent === "pharmacy") {
      //   if (key >= "1" && key <= "9") {
      ...
      //       toast.info(`Category: ${categories[categoryIndex - 1]}`);
      //       return false;
      //     }
      //   }
      // }

      // ====== Enter Key - Add Selected Item ======
      if (event.key === "Enter" && activeBlock === 'itemsPOS' && selectedItemIndex >= 0 && checkComponent === "pharmacy") {
        const activeElement = document.activeElement;
        if (activeElement === searchRef.current) {
          event.preventDefault();
          event.stopPropagation();
      ...
      // ====== Escape Key - Clear/Reset ======
      if (event.key === "Escape" && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setSelectedItemIndex(-1);
        setActiveFieldIndex(0);
      ...
      // ====== Plus/Minus Keys - Adjust Quantity ======
      // if ((key === "+" || key === "=") && activeBlock === 'invoiceItems' && checkComponent === "pharmacy") {
      //   event.preventDefault();
      //   event.stopPropagation();
      //   const rowIndex = Math.floor(activeFieldIndex / 3);
      //   if (rowIndex >= 0 && rowIndex < tableData.length) {
      //     const newData = [...tableData];
      //     const currentQty = parseFloat(newData[rowIndex].quantity) || 0;
      //     newData[rowIndex].quantity = (currentQty + 1).toString();
      //     const price = parseFloat(newData[rowIndex].price) || 0;
      //     const discount = parseFloat(newData[rowIndex].discount) || 0;
      //     newData[rowIndex].rate_after_discount = (price * (currentQty + 1) - discount).toFixed(2);
      //     newData[rowIndex].total = newData[rowIndex].rate_after_discount;
      //     setTableData(newData);
      //   }
      //   return false;
      // }
  ...
  if (rowIndex >= 0 && rowIndex < tableData.length) {
    ...
    newData[rowIndex].quantity = newQty.toString();
    const price = parseFloat(newData[rowIndex].price) || 0;
    const discount = parseFloat(newData[rowIndex].discount) || 0;
    newData[rowIndex].rate_after_discount = (price * newQty - discount).toFixed(2);
    newData[rowIndex].total = newData[rowIndex].rate_after_discount;
    setTableData(newData);
  ...
  return false;
      ...
      // if (key === "-" && activeBlock === 'invoiceItems' && checkComponent === "pharmacy") {
      //   event.preventDefault();
      //   event.stopPropagation();
      //   const rowIndex = Math.floor(activeFieldIndex / 3);
      //   if (rowIndex >= 0 && rowIndex < tableData.length) {
...
if (key === "-" && activeBlock === 'invoiceItems' && checkComponent === "pharmacy") {
  ...
  return false;
      ...
      // Navigation without Ctrl for non-input elements
      if (event.key === 'ArrowLeft' && !isInInput && !ctrl && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock(prev => {
          if (prev === 'invoiceItems') return 'itemsPOS';
      ...
      if (event.key === 'ArrowRight' && !isInInput && !ctrl && checkComponent === "pharmacy") {
        event.preventDefault();
        event.stopPropagation();
        setActiveBlock(prev => {
          if (prev === 'invoiceForm') return 'itemsPOS';
      ...
      if (activeBlock === 'invoiceForm' && isInInput && checkComponent === "pharmacy") {
      ...
      // Items POS Navigation
      // if (activeBlock === 'itemsPOS' && checkComponent === "pharmacy") {
      //   const itemCount = filteredItems.length;
      ...
      // Items POS Navigation - Replace existing code in your useEffect
if (activeBlock === 'itemsPOS' && checkComponent === "pharmacy") {
  const itemCount = filteredItems.length;
  const itemsPerRow = Math.floor((window.innerWidth - 400) / 196); // Calculate items per row (180px + 16px gap)
  ...
  if (activeElement === barcodeRef.current) {
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      ...
      // Invoice Items Navigation
      // if (activeBlock === 'invoiceItems' && isInInput && checkComponent === "pharmacy") {
      //   const currentRow = Math.floor(activeFieldIndex / 3);
      //   const currentCol = activeFieldIndex % 3;
      //   const totalRows = tableData.length;
      ...
      // Invoice Items Navigation - Replace the existing cart items navigation code
if (activeBlock === 'invoiceItems' && isInInput && checkComponent === "pharmacy") {
  const currentRow = Math.floor(activeFieldIndex / 2); // Changed from 3 to 2 (only Qty and Disc fields)
  const currentCol = activeFieldIndex % 2;
  const totalRows = tableData.length;
  ...
  if (event.key === 'ArrowDown') {
    ...
    event.stopPropagation();
    if (currentRow < totalRows - 1) {
      setActiveFieldIndex(prev => prev + 2); // Changed from 3 to 2
    }
    return false;
  } else if (event.key === 'ArrowUp') {
    event.preventDefault();
    ...
    // Only move if cursor is at the end of input
    if (event.target.selectionStart === event.target.value.length) {
      if (currentCol < 1) { // Changed from 2 to 1 (only 2 fields now)
        event.preventDefault();
        event.stopPropagation();
        setActiveFieldIndex(prev => prev + 1);
        return false;
    ...
    // Only move if cursor is at the start of input
  ...
  const toggleContactMethod = () => {
    setContactMethod((prev) => (prev === "phone" ? "whatsapp" : "phone"));
    toast.info(`Contact Method: ${contactMethod === "phone" ? "WhatsApp" : "Phone"}`);
  };
...
.shortcuts-help::-webkit-scrollbar {
  display: none;
}
...
.section-indicator {
  ...
  .section-item.active .check-icon {
    ...
    color: #10b981;
      ...
      <div className="d-flex justify-content-start flex-wrap mt-2 pl-4">
        <a href="#" className="btn btn-sm btn-warning mb-2 mr-2">
          <Link to="/stock">
            <i className="fas fa-truck"></i> Create Stock
          </Link>
        ...
        <>
          ...
          <div className="shortcuts-help">
            <div className="shortcuts-header">
              ...
              <button onClick={() => setShowShortcutsHelp(false)} className="close-btn" style={{color: '#000'}}>
                <i className="fas fa-times" style={{fontSize: '24px'}}></i>
              </button>
            </div>
            ...
            <div className="shortcuts-grid">
              <div>
                <div className="shortcuts-section">
                  ...
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Save Invoice</span>
                    <span className="shortcut-key">Ctrl + S</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">New Invoice</span>
                    <span className="shortcut-key">Ctrl + N</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Open Invoices</span>
                    <span className="shortcut-key">Ctrl + I</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Print Invoice</span>
                    <span className="shortcut-key">Ctrl + P</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Clear Cart</span>
                ...
                <div className="shortcuts-section">
                  ...
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Focus Barcode</span>
                    <span className="shortcut-key">Ctrl + B</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Focus Phone</span>
                    <span className="shortcut-key">Ctrl + U</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Focus Name</span>
                ...
                <div className="shortcuts-section">
                  <h3>🎯 Function Keys</h3>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Toggle Checkout</span>
                    <span className="shortcut-key">F9</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Payment Status</span>
                    <span className="shortcut-key">F2</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Focus Advance</span>
                    <span className="shortcut-key">F3</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Contact Method</span>
              ...
              <div>
                <div className="shortcuts-section">
                  <h3>🧭 Navigation</h3>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Navigate Sections</span>
                    <span className="shortcut-key">Ctrl + ← →</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Move in Fields</span>
                    <span className="shortcut-key">↑ ↓ ← →</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Add Selected Item</span>
                    <span className="shortcut-key">Enter</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Cancel/Clear</span>
                ...
                <div className="shortcuts-section">
                  ...
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Delete Item</span>
                    <span className="shortcut-key">Ctrl + X</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Increase Qty</span>
                    <span className="shortcut-key">+</span>
                  </div>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Decrease Qty</span>
                    <span className="shortcut-key">-</span>
                  </div>
                </div>
                ...
                <div className="shortcuts-section">
                  <h3>🏷️ Categories</h3>
                  <div className="shortcut-item">
                    <span className="shortcut-desc">Quick Category</span>
      ...
      <div className="modern-pos-container">
        ...
        <div className="pos-main-grid">
          ...
          <div style={{position: 'sticky', top: 0, background: 'white', zIndex:10, paddingTop: '0'}}>
            <div className="flex pt-2 mb-2 pb-0 pl-3 bg-warning">
                  <label className="flex items-center gap-2">
                    <input
                      type="radio"
            ...
            <div className="category-tabs">
              {categories.map((category, idx) => (
                <button
                  ...
                  className={`category-tab ${selectedCategory === category ? 'active' : ''}`}
                  disabled={formDisabled}
                  title={`Press ${idx + 1} for quick select`}
                >
                  {category}
                </button>
              ))}
              ...
              {filteredItems.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* <div className="product-image">💇‍♂️✂️</div> */}
  ...
  <div className={`section-item ${activeBlock === 'invoiceForm' ? 'active' : ''}`}>
    <i className="fas fa-check-circle check-icon"></i>
    <span>Client</span>
  </div>
  <div className={`section-item ${activeBlock === 'itemsPOS' ? 'active' : ''}`}>
    <i className="fas fa-check-circle check-icon"></i>
    <span>Items</span>
  </div>
  <div className={`section-item ${activeBlock === 'invoiceItems' ? 'active' : ''}`}>
    <i className="fas fa-check-circle check-icon"></i>
                ...
                <div className="cart-items">
                  {tableData.length === 0 ? (
                    <div className="cart-empty">
                      <i className="fas fa-shopping-cart" style={{ fontSize: '64px', marginBottom: '16px' }}></i>
                      <p>No items in cart</p>
                      <p style={{fontSize: '0.875rem', marginTop: '8px'}}>Press Ctrl+H for shortcuts</p>
                    </div>
                  ) : (
                    tableData.map((item, index) => (
                      <div key={index} className="cart-item">
                        ...
                        <div className="cart-item-controls">
                          {/* <input
                            ...
                            placeholder="Qty"
                            ref={(el) => {
                              if (activeBlock === 'invoiceItems') {
                                invoiceItemsRefs.current[index * 3] = el;
                              }
                            }}
                          />
                          <input
                            ...
                            ref={(el) => {
                              if (activeBlock === 'invoiceItems') {
                                invoiceItemsRefs.current[index * 3 + 1] = el;
                              }
                            }}
                          /> */}
...
<input
  ...
  ref={(el) => {
    if (activeBlock === 'invoiceItems') {
      invoiceItemsRefs.current[index * 2] = el; // Changed from index * 3 to index * 2
    }
  }}
/>
<input
  ...
  ref={(el) => {
    if (activeBlock === 'invoiceItems') {
      invoiceItemsRefs.current[index * 2 + 1] = el; // Changed from index * 3 + 1 to index * 2 + 1
    }
  }}
/>
                          <div className="cart-item-total">Rs. {item.total}</div>
              ...
              <div className="checkout-view">
                ...
                <div className="checkout-header">
                  <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700 }}>Finalize Sale</h2>
                  <button onClick={() => setShowCheckout(false)} className="close-btn" title="Back (Esc)">
                    <i className="fas fa-times" style={{ fontSize: '24px' }}></i>
                  </button>
                </div>
                ...
                <div className="checkout-tabs">
                  ...
                  <button className="checkout-tab d-none">
                    <i className="fas fa-user mr-2"></i>
                    Customer Details
                  </button>
                </div>
                ...
                <div className="checkout-content">
                  ...
                  <div className="checkout-section">
                    ...
                    <div className="form-group">
                      <label className="form-label">Phone Number (Ctrl+U)</label>
                      <input
                        ref={phoneRef}
                        type="text"
                        value={formData.phone_no}
                    ...
                    <div className="form-group">
                      <label className="form-label">Full Name (Ctrl+F)</label>
                      <input
                        ref={fullNameRef}
                        type="text"
                        value={formData.full_name}
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        <label className="form-label">Status (F2 to toggle)</label>
                        <select
                ...
                <div className="checkout-footer">
                  <div className="checkout-summary">
                    ...
                    <div className="form-group">
                      <label className="form-label">Advance / Paid (F3 to focus)</label>
                      <input
                        ref={advanceRef}
                        type="number"
                        value={formData.advance}
                    ...
                    <div className="checkout-summary-row">
                      <span>Remaining</span>
                      <span className="remaining-amount">Rs. {remaining.toFixed(2)}</span>
                    </div>
                  </div>
