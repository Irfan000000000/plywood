...
function AddLabTest() {
  ...
  const [showEdit, setShowEdit] = useState(null); // Initialize as null or false
  ...
  const fetchData = () => {
    axios.get(process.env.REACT_APP_API_URL+"/lab_tests", {
      params: {
        page: currentPage,
        limit: totalItem,
  ...
  function fetchSuggestionList(){
    axios.get(process.env.REACT_APP_API_URL+"/get-all-lab-test-for-suggestion", {
      params: {
        page: currentPage,
        limit: totalItem
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (editFormData.hidden_id !== "") {
      ...
      axios.put(`${process.env.REACT_APP_API_URL}/update-lab-test/${itemId}`, editFormData)
        .then(response => {
      ...
      // Insert new item
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+'/insert_lab_test', {
          method: 'POST',
          headers: {
  ...
  const editItem = (item_id_get) => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/lab-test-get/${itemId}`)
      .then(response => {
        const { id, lab_test } = response.data.results[0];
        setEditFormData({
          item_name: lab_test || '',
          hidden_id: id || ''
        });
  ...
  function AddLabTest() {
    setShowEdit('add_lab_test_heads'); // Update the same variable
  }
  
  // // In your JSX:
  // {showEdit === 'add_lab_test_heads' && (
  ...
  //           </button>
  //           </div>
  //           <LabTestHeads onClose={() => setShowEdit(false)}/>
  //         </div>
  // )}
      ...
      <div className="d-flex">
        ...
        <div className='col-md-6 p-2'>
          <h5 className='text-warning bg-primary p-2 card-header border'>
            <i className="fas fa-first-aid"></i> Clinical Head Form
          </h5>
          <div className='pt-2 pb-2'>
          <a href="#" className='btn btn-sm btn-warning mb-2 mr-2'  onClick={() => AddLabTest()}><i className="fas fa-flask"></i><i className="fas fa-plus"></i> Add Lab Test Heads</a>
          </div>
          <div>
        ...
        <div className="col-md-6 p-2">
          <div className="card-header text-warning bg-primary p-2">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                <i className="fas fa-list"></i> Lab Test List
              </div>
            </div>
          </div>
